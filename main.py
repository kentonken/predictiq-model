...
async def startup():
    try:
        ...
        if model:
            ...
            _training_status = {"status": "complete", "message": "Model active."}
    except Exception as e:
        logger.warning(f"⚠️ Model not found on startup: {e}. This is expected before first /train.")

# --- Background Training Job ---
def _run_training_job(date_from: str, date_to: str):
    global _training_status
    _training_status = {"status": "running", "message": "Fetching historical data from Bzzoiro..."}
    ...
    try:
        ...
        if not rows or len(rows) < 5:
            _training_status = {"status": "error", "message": "Insufficient data for training."}
            return

        df = pd.DataFrame(rows)
        _training_status["message"] = f"Training ensemble on {len(df)} matches..."
        ...
        if upload_model(model):
            ...
            _training_status = {"status": "complete", "message": "Model trained and deployed successfully."}
        else:
...
@app.get("/health")
async def health():
    return {"status": "ok", "model_active": _training_status["status"] == "complete"}
